 ...
 *
 * Stands in for the real sync filters, which serve a node from a local cache keyed on its hash and
 * so say nothing about where in a tree it belongs.
 */
...
protected:
    ...
    /**
     * A genuine leaf holding kKey, in the form a sync filter serves, with its hash.
     *
     * Taken from a map that placed the leaf correctly, so only its position is ever wrong below.
     * Serialized with its prefix rather than in wire form, since that is what checkFilter parses.
     *
     * @param f the family the throwaway source map belongs to.
     * @return the leaf's prefixed form and its hash, or an empty blob if the map rejected the item.
     */
    ...
    /**
     * Assemble `map` as a root inner node holding a leaf's hash under the wrong branch.
     *
     * The root is installed directly, as a peer's would be, so the leaf itself stays unresolved
     * until a walk consults the filter for it.
     *
