...
public:
    ...
    // Targeted unit test for loanDefaultFreezeExemptAccounts(): builds a real
    ...
    testLoanDefaultFreezeExemptAccounts()
    {
        ...
        testcase("loanDefaultFreezeExemptAccounts");
        ...
        // Not a LoanManage transaction at all.
        {
            ...
            BEAST_EXPECT(!loanDefaultFreezeExemptAccounts(*env.current(), *jt.stx));
        }
        ...
        // resolves the issuer, broker, and vault accounts.
        {
            auto const jt = env.jt(manage(lender, loanKeylet.key, tfLoanDefault));
            auto const result = loanDefaultFreezeExemptAccounts(*env.current(), *jt.stx);
