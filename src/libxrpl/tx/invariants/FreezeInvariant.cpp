    ...
    beast::Journal const& j)
{
     ...
     * loanDefaultFreezeExemptAccounts), so a frozen issuer would otherwise
     * trip this invariant on either hop. Gated behind fixCleanup3_4_0, and
     * scoped to exactly the issuer/broker and issuer/vault lines involved, so
     * ledgers without the amendment (or unrelated frozen lines touched by the
     * same transaction) keep the current (blocking) behavior.
     */
    ...
    return std::ranges::all_of(balanceChanges_, [&](auto const& entry) {
        ...
        return validateIssuerChanges(issuerSle, changes, tx, j, enforce, loanDefaultAccounts);
    });
}
    ...
    for (auto const& actors : {changes.senders, changes.receivers})
    {
        for (auto const& change : actors)
        {
            ...
            if (!validateFrozenState(
                    change, high, tx, j, enforce, globalFreeze, loanDefaultAccounts))
            {
                return false;
            }
        }
    ...
    std::optional<LoanDefaultFreezeExemptAccounts> const& loanDefaultAccounts)
{
    ...
    // issuer/vault lines are exempt -- an unrelated frozen line touched by
    // the same transaction is still caught.
    if (loanDefaultAccounts)
    {
